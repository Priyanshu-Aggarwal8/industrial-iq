/**
 * LAYER 3: DOMAIN - ACTIONABLE INSIGHT ENGINE
 * Deterministic detection and scoring of operational risks and bottlenecks.
 * Zero black-box AI, zero hallucinated data. Traceable to exact source records.
 */

import { NormalizedLead } from '../data/normalizer';
import { RawBranch, RawSalesRep, RawTarget, RawDelivery } from '../data/schemas';
import { DomainActionableInsight } from './models';
...
export function generateActionableInsights(
  ...
  const insights: DomainActionableInsight[] = [];
  ...
  const deliveryAnalytics = calculateDeliveryAnalytics(
    deliveries,
    leads,
    branches,
    startDate,
  ...
  // 1. Critical Branch Target Deficit Alert
  ...
  // 2. Early-Funnel Drop-off Breakdown Alert
  for (const b of branchSummaries) {
    if (selectedBranchId && b.branch.id !== selectedBranchId) continue;

    if (b.conversionRate < 15 && b.leadsCreated >= 20) {
      const branchLeads = leads.filter(l => l.branch_id === b.branch.id);
  ...
  const staleHighValueLeads = leads.filter(
    l =>
  ...
  // 6. Marketing Channel Conversion Inefficiency Alert
